...
def main(req: func.HttpRequest) -> func.HttpResponse:
    logging.info(
        'sosi_func0003_crawler_stock_listing_details function processed a request.')
    ...
    try:
        ...
        if (next_service_url == ''):
            msg = "No service URL found. {} was processed but no further action was taken. This service is ready for another request.".format(
                stock_obj.code)

            logging.warning(msg)
            return func.HttpResponse(body=msg, status_code=200)
        else:
            threading.Thread(target=invoke_url, args=(
                next_service_url, json_obj, func_key_header)).start()
            msg = "'{}' was sent to next step. This service is ready for another request".format(
                stock_obj.code)
